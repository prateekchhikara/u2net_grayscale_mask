...
def training_loop(opt):
    ...
    # from pcLoader import returnDataLoader
    # loader = returnDataLoader()

    from data.aligned_dataset import AlignedDataset

    custom_dataloader = AlignedDataset(opt, "train")
    loader = torch.utils.data.DataLoader(custom_dataloader, batch_size=1, shuffle=True)
    ...
    # loss function
    weights = np.array([1, 1.5, 1.5, 1.5, 1.5], dtype=np.float32)
    ...
    for itr in pbar:
        ...
        # change label data type to float
        label = label.squeeze(1)
        label = label.type(torch.long)
        
        # label = torch.randint(low=0, high=3, size=(1, label.shape[2], label.shape[3])).to(device)
        ...
        # print(total_loss.item()/dataset_size)
